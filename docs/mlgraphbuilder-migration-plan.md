...
**Acceptance:** Generated tests under `tests/onnx_ops/` for these op families reach `build()` Ok (`cargo test --test onnx_op_tests`).
...
**Keep:** `scripts/webnn_onnx_ops.py`, `scripts/onnx_ops_to_csv.py`, `scripts/generate_rust_op_conversion_tests.py` (and its builder/emitter helpers; op support manifest unchanged).
...
1. Integration tests: `tests/onnx_ops/` (generated by `scripts/generate_rust_op_conversion_tests.py`); each test builds an in-memory `ModelProto` and expects `convert_onnx` + ORT `build()` Ok for supported ops.
2. Update `AGENTS.md` and `README.md`: validation = ORT build; export deferred.
3. Update `efficientnet.cmd`: convert step = ORT validation only (no `.webnn` artifact; use `scripts/` paths for Python helpers).
...
- [ ] Supported op conversion tests in `tests/onnx_ops/`: `build()` Ok
